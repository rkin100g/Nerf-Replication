...
from src.utils import base_utils
import numpy as np
import os
import imageio
import cv2
...
class Dataset(data.Dataset):
    def __init__(self, **kwargs):
        ...
        start_frames, end_frames, skip_frames = (
            kwargs["frames"][0],
            kwargs["frames"][1],
            kwargs["frames"][2],
        )
        ...
        for frame_index in np.arange(num_frames)[start_frames:end_frames:skip_frames]:
                ...
                item.update(
                    {
                        "img_path": os.path.join(
                            self.data_root,
                            annots["ims"][frame_index]["ims"][camera_index],
                        )
                    }
                )
                item.update(
            ...
            vertices_start = (
                1 if "313" in self.data_root or "315" in self.data_root else 0
            )
            vertices = np.load(
                os.path.join(
                    self.data_root,
                    vertices_dir,
                    "{}.npy".format(frame_index + vertices_start),
                )
            )
            self.bboxs.update(
                {
                    frame_index: np.concatenate(
                        [
                            np.min(vertices, axis=0) - 0.05,
                            np.max(vertices, axis=0) + 0.05,
                        ]
                    )
                }
            )
        ...
        self.wbbox = np.concatenate(
            [np.min(bboxs[:, :3], axis=0), np.max(bboxs[:, 3:6], axis=0)]
        )
    ...
    def get_mask(self, item):
        ...
        msk_path = os.path.join(self.data_root, "mask_cihp", img_path)[:-4] + ".png"
        if not os.path.exists(msk_path):
            msk_path = os.path.join(self.data_root, "mask", img_path)[:-4] + ".png"
        if not os.path.exists(msk_path):
            msk_path = (
                os.path.join(self.data_root, img_path.replace("images", "mask"))[:-4]
                + ".png"
            )
    ...
    def read_data(self, item, index):
        ...
        if index in self.caches:
            ...
            img = imageio.imread(item["img_path"]).astype(np.float32) / 255.0
            H, W = img.shape[:2]
            ...
            D, K = (
                np.array(self.cams["D"][item["camera_index"]]),
                np.array(self.cams["K"])[item["camera_index"]],
            )
        ...
        K, R, T = (
            np.array(self.cams["K"][camera_index]).copy(),
            np.array(self.cams["R"][camera_index]),
            np.array(self.cams["T"][camera_index]) / 1000.0,
        )
        if self.input_ratio != 1.0:
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            pixel_x = np.random.randint(
                low=max(int(bbox[0]), 0), high=min(int(bbox[2]) + 1, W), size=4 * fg_num
            )
            pixel_y = np.random.randint(
                low=max(int(bbox[1]), 0), high=min(int(bbox[3]) + 1, H), size=4 * fg_num
            )
            mask_val = mask[pixel_y, pixel_x]
            pixel_x = pixel_x[mask_val == 1][:fg_num]
            pixel_y = pixel_y[mask_val == 1][:fg_num]

            mask = base_utils.get_bound_2d_mask(
                self.wbbox.reshape((2, 3)), ixt, ext, H, W
            )
            bbox = base_utils.get_bbox_2d(
                self.wbbox.reshape((2, 3)), ixt, ext
            )  # min_x, min_y, max_x, max_y
            pixel_x_ = np.random.randint(
                low=max(int(bbox[0]), 0), high=min(int(bbox[2]) + 1, W), size=3 * bg_num
            )
            pixel_y_ = np.random.randint(
                low=max(int(bbox[1]), 0), high=min(int(bbox[3]) + 1, H), size=3 * bg_num
            )
            mask_val = mask[pixel_y_, pixel_x_]
            pixel_x_ = pixel_x_[mask_val == 1][:bg_num]
            pixel_y_ = pixel_y_[mask_val == 1][:bg_num]
            pixel_x, pixel_y = np.concatenate([pixel_x, pixel_x_]), np.concatenate(
                [pixel_y, pixel_y_]
            )
        ...
        ret = {
            ...
            "rgb": rgb,
            "wbounds": self.wbbox.astype(np.float32),
        }
