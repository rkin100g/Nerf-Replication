...
class Motion2d(nn.Module):
    ...
    def forward(self, xyzt, wbounds):
        ...
        if xyzt[0, 3] != 0:
            ...
            xyz = torch.clamp(inputs[..., :3] + 2 * delta_xyz - 1.0, min=0.0, max=1.0)
...
class HashEncoderCoef(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.basis = nn.ModuleList(
            [HashEncoder(**kwargs) for i in range(self.basis_num)]
        )
        kwargs["input_dim"], kwargs["log2_hashmap_size"] = 4, 20
        self.coefs = HashEncoder(**kwargs)
        self.coefs_mlp = nn.ModuleList(
            [nn.Linear(self.coefs.out_dim, 64), nn.Linear(64, self.basis_num)]
        )
...
class DNeRFNGP(nn.Module):
    ...
    def forward(self, x, wbounds=None):
        ...
        if t[0] == 0.0:
            ...
            return self.encoder(
                torch.clamp(inputs + delta_xyz, min=0.0, max=1.0), normalize=False
            )
